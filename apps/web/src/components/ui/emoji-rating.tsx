...
const ratingData = [
  ...
  { emoji: "😍", label: "Amazing", color: "from-emerald-400 to-emerald-500", shadowColor: "shadow-emerald500/30" },
]
  ...
  return (
    <div className={cn("flex flex-col items-center gap-6", className)}>
      ...
      <div className="relative h-7 w-32">
        ...
        >
          <span className="text-sm font-medium" style={{ color: "var(--text-muted)" }}>Rate us</span>
        </div>
        ...
        {ratingData.map((item, i) => (
          ...
          >
            <span className="text-sm font-semibold tracking-wide" style={{ color: "var(--text-primary)" }}>{item.label}</span>
          </div>
        ))}
      </div>
    </div>
